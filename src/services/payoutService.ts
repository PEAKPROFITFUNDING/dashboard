...
export function fetchTraderAccounts(): Promise<TraderAccount[]> {
    return new Promise((resolve) => setTimeout(() => resolve(dummyTraderAccounts), 800));
  }
...
export function fetchTraderChallenges(): Promise<Challenge[]> {
  return new Promise((resolve) => setTimeout(() => resolve(dummyChallenges), 800));
}

// Mock API: fetch payout eligibility for an account
export function fetchPayoutEligibility(accountId: string): Promise<PayoutEligibility> {
...
export function submitPayoutRequest(request: PayoutRequest): Promise<{ success: boolean; message: string }> {
  return new Promise((resolve) =>
    setTimeout(() => resolve({ success: true, message: 'Payout request submitted successfully!' }), 1000)
  );
}
